...
// --- OpenAPI / Swagger upload wiring (minimal, non-invasive) ---
const sdk = useSDK();
...
const onFileSelected = async (ev: Event) => {
  ...
  try {
    ...
    const created = await sdk.backend.importOpenApi(text);
    sdk.window.showToast(`Imported ${created.length} templates`, { variant: "success" });
    // backend emits templates:created events which should populate the UI/store
  } catch (e) {
    sdk.window.showToast((e as Error).message ?? "Failed to import OpenAPI file", {
      variant: "error",
    });
  } finally {
      ...
      <template #header>
        ...
        >
          ...
          <div class="flex items-end gap-4">
            ...
            <!-- Import OpenAPI / Swagger UI -->
            <div class="flex items-center gap-2">
              <input
                ref="openapiInput"
                type="file"
                accept=".json,application/json,.yaml,.yml,application/x-yaml"
                class="hidden"
